...
function formatduration(duration) {
  ...
  var ds = formatNumber(duration.getMinutes()) + ":" + formatNumber(duration.getSeconds());
  return ds;
}
//转换歌词字符串为数组
...
function playAlrc(that, app) {
  ...
  if (that.data.music.id != app.globalData.curplay.id) {
    that.setData({
      ...
      showlrc: false,
      // lrcindex: 0,
      // duration: formatduration(app.globalData.curplay.timeTotal * 1000)
    });
  ...
  wx.getBackgroundAudioPlayerState({
    complete: function (res) {
      ...
      if (res.status != 2) {
        ...
        playtimeTime = res.currentPosition * 1000;
      ...
      that.setData({
        playtime: formatduration(playtimeTime),
        playtimeTime:playtimeTime,
        percent: time,
        playing: playing,
        // lrcindex: lrcindex
      })
